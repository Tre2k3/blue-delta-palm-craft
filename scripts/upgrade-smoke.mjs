...
const url = new URL(process.env.GAME_URL || "http://127.0.0.1:8080/");
...
async function open(page) {
  ...
  await page
    .getByRole("button", { name: "ENTER MEMPHIS", exact: true })
  ...
  await page.evaluate(() => {
    const e = window.__gameTest.getEngine();
    e.running = false;
...
async function step(page, seconds = 0.05) {
  await page.evaluate((t) => {
    window.__gameTest.step(t);
    window.__gameTest.getEngine().draw();
  }, seconds);
}
...
async function travel(page, id) {
  await page.evaluate((id) => window.__gameTest.teleport(id), id);
  await step(page);
}
...
async function dialogue(page) {
  for (let i = 0; i < 12 && (await state(page)).mode === "dialogue"; i++)
    await page.evaluate(() => window.__gameTest.advanceDialogue());
  await step(page);
}
async function greenShot(page) {
  await page.keyboard.down("KeyF");
  await step(page, 0.83);
  const charged = await state(page);
  assert.ok(charged.power >= 0.54 && charged.power <= 0.76, "charge reaches the green window");
  await page.keyboard.up("KeyF");
  await step(page, 1.45);
  const shot = await state(page);
  assert.equal(shot.shots, charged.shots + 1, "one release produces one shot");
  assert.ok(shot.score > charged.score, "actual ball arc scores");
  await step(page, 1);
  assert.equal((await state(page)).score, shot.score, "basket is counted once");
}
...
try {
  ...
  await travel(page, "store");
  await interact(page);
  await dialogue(page);
  assert.equal((await state(page)).step, "pickup");
  await travel(page, "dropvan");
  await interact(page);
  ...
  assert.ok(
    (await page.evaluate(() => window.__controlsTest.getYaw())) > beforeYaw,
    "A steers left under chase camera",
  );
  ...
  assert.ok(
    (await page.evaluate(() => window.__controlsTest.getYaw())) < leftYaw,
    "D steers right under chase camera",
  );
  ...
  for (const id of ["neighborhood", "downtown", "culture"]) {
    await travel(page, id);
    await interact(page);
  }
  assert.equal((await state(page)).step, "ball");
  await travel(page, "court");
  await interact(page);
  ...
  await page.getByRole("button", { name: "Leave court", exact: true }).click();
  assert.equal(
    (await state(page)).sackdollars,
    paid.sackdollars,
    "empty round cannot reuse an earlier streak bonus",
  );
  await travel(page, "store");
  await interact(page);
  await dialogue(page);
  assert.equal(
    (await state(page)).missionComplete,
    true,
    "Drop Day completes through real interactions and shots",
  );
  await page.evaluate(() => {
    const e = window.__gameTest.getEngine();
    e.save();
  ...
  assert.equal(restored.x, parked.x);
  assert.equal(restored.y, parked.y);
  ...
  await page.close();
  console.log(
    "PASS: movement, map, mission, physical shots, payouts, van, reload and reset protection",
  );
  ...
  await open(mobile);
  const stick = mobile.getByRole("group", { name: "Movement joystick" });
  ...
  await mobile.mouse.move(rect.x + rect.width / 2, rect.y + rect.height / 2 - 35);
  await step(mobile, 0.5);
  await mobile.mouse.up();
  await step(mobile);
  assert.ok(
    Math.hypot((await state(mobile)).px - before.px, (await state(mobile)).py - before.py) > 20,
    "touch joystick moves the player",
  );
  assert.equal(
    await mobile.evaluate(() => window.__gameTest.getEngine().input.touch.my),
    0,
  ...
  console.log(
    "PASS: mobile movement, release, map and portrait/landscape layout; no browser errors",
  );
} finally {
  await closeBrowser(browser);
  clearTimeout();
