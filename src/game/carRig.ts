...
import { TRAFFIC_WRAPS, bindWrapTextures, kindFromPack, DROP_VAN_WRAP, type CarKind, type WrapPackName } from "./vehicleWraps";
import { WHEEL_RADIUS, buildCarBody } from "./carBody";
...
export function makeStreetCar(index: number, parked = false): StreetCarRig {
  ...
  const car = buildCarBody(look.kind, { color: look.color, pack: look.pack, parked });
  car.root.name = parked ? "parked-street-car" : "world-life-3d-vehicle";
  if (parked && look.pack) bindWrapTextures(car.hull);
  return {
    ...car,
    pack: look.pack,
    kind: look.kind,
...
export function makeDropVan(): THREE.Group {
  const van = buildCarBody("van", { pack: DROP_VAN_WRAP, L: 3.12, W: 1.28 });
  van.root.name = "drop-van";
  for (const mat of van.headlights) mat.emissiveIntensity = 0.7;
  for (const mat of van.brakes) mat.emissiveIntensity = 0.28;
  bindWrapTextures(van.hull);
  return van.root;
}
