...
export function shotVelocity(x: number, y: number, yaw: number, power: number, hoop: { x: number; y: number; z: number }) {
  ...
  return { vx: -Math.sin(angle) * speed, vy: -Math.cos(angle) * speed,
    vz: (hoop.z - 42 + 0.5 * GRAVITY * duration ** 2) / duration, duration, distance,
    grade: perfect ? "PERFECT" : good ? "GOOD" : power < 0.42 ? "EARLY" : "LATE" } as const;
}
/** Swept crossing prevents both tunnelling and scoring the same basket twice. */
export function crossesHoop(before: { x: number; y: number; z: number }, after: { x: number; y: number; z: number }, hoop: { x: number; y: number; z: number }) {
  if (before.z <= hoop.z || after.z > hoop.z) return false;
  const t = (before.z - hoop.z) / (before.z - after.z);
  return Math.hypot(before.x + (after.x - before.x) * t - hoop.x, before.y + (after.y - before.y) * t - hoop.y) < 11;
}
export function driveStep(speed: number, yaw: number, steer: number, throttle: number, brake: boolean, dt: number) {
