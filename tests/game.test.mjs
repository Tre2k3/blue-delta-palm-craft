  ...
  for (const distance of [65, 155, 252, 320])
    for (const angle of [-3.13, -1, 0, 1, 3.13]) {
      ...
      for (let t = 0; t < 1.8; t += FIXED_STEP) {
        const after = {
          x: ball.x + shot.vx * FIXED_STEP,
          y: ball.y + shot.vy * FIXED_STEP,
          z: ball.z + vz * FIXED_STEP - 0.5 * GRAVITY * FIXED_STEP ** 2,
        };
        crossings += Number(crossesHoop(ball, after, hoop));
        vz -= GRAVITY * FIXED_STEP;
        ball = after;
      }
...
test("hoop crossings reject upward shots and wide misses, handle fast downward shots", () => {
  ...
  assert.equal(shotVelocity(100, 300, 0, 0.1, hoop).grade, "EARLY");
...
test("van accelerates, reverses, steers both directions, and brakes independently of frame rate", () => {
  ...
  let small = { speed: 0, yaw: 0 };
  for (let i = 0; i < 60; i++) small = driveStep(small.speed, small.yaw, 0, 1, false, FIXED_STEP);
...
test("legacy saves preserve earned progress and migrate safely", () => {
  ...
  const save = parseSave(
    JSON.stringify({
      ...
      sackdollars: 123,
      respect: 45,
      owned: ["starter_tee", "black_hoodie"],
      equipped: "black_hoodie",
      missionProgress: { [steps[0].id]: true, [steps[1].id]: true },
      basketballHighScore: 24,
      trophies: ["family"],
    }),
  );
  assert.equal(save.version, 3);
  assert.equal(save.sackdollars, 123);
  assert.equal(save.equipped, "black_hoodie");
  assert.equal(save.missionActiveStep, 2);
  assert.equal(save.missionComplete, false);
  assert.equal(save.basketballHighScore, 24);
  assert.deepEqual(save.trophies, ["family"]);
  assert.equal(save.position.y, 558);
  assert.equal(save.vehicle.x, 1896);
});
...
test("invalid saves, IDs, values and nonsequential objectives cannot corrupt game state", () => {
  ...
  const save = parseSave(
    JSON.stringify({
      ...
      sackdollars: -50,
      respect: "infinite",
      owned: ["fake", "black_hoodie", "black_hoodie"],
      equipped: "gold_chain",
      missionActiveStep: 900,
      missionComplete: true,
      missionProgress: { [steps[2].id]: true },
      position: { x: 1e9, y: -10 },
      talked: ["fake", NPCS[0].id, NPCS[0].id],
    }),
  );
  assert.equal(save.sackdollars, 0);
  assert.equal(save.respect, 0);
  assert.deepEqual(save.owned, ["starter_tee", "black_hoodie"]);
  assert.equal(save.equipped, "starter_tee");
  assert.equal(save.missionActiveStep, 0);
  assert.equal(save.missionComplete, false);
  assert.ok(save.position.x < 4000);
  assert.equal(save.position.y, 62);
  assert.deepEqual(save.talked, [NPCS[0].id]);
  assert.deepEqual(sanitizeSettings({ master: Infinity, music: -1, sfx: 5, sensitivity: 0 }), {
    master: 0.85,
    music: 0,
    sfx: 1,
    sensitivity: 0.25,
...
test("saved exploration and parked van survive a round trip", () => {
  const save = parseSave(
    JSON.stringify({
      version: 3,
      position: { x: 200, y: 650, yaw: 1 },
      vehicle: { x: 1550, y: 1070, yaw: -1 },
      visited: ["court", "court", "fake"],
      talked: [NPCS[0].id],
    }),
  );
  assert.deepEqual(parseSave(JSON.stringify(save)), save);
  assert.deepEqual(save.visited, ["court"]);
...
test("landmarks, entrances and mission NPCs keep roads and solid buildings navigable", () => {
  for (const p of POIS.filter((p) => p.id !== "river" && p.id !== "dropvan")) {
    assert.equal(
      roadRects().some((r) => overlaps(p, r)),
      false,
      `${p.id} blocks a road`,
    );
    const entry = poiEntrance(p.id);
    assert.equal(
      poiColliders().some((r) => circleHitsRect(entry.x, entry.y, 14, r)),
      false,
      `${p.id} entrance blocked`,
    );
  }
  for (const npc of NPCS)
    assert.equal(
      poiColliders().some((r) => circleHitsRect(npc.x, npc.y, 14, r)),
      false,
      `${npc.id} inside a building`,
    );
  for (const lane of trafficLanes())
    assert.equal(
      isRoadPoint(lane.axis === "x" ? 100 : lane.fixed, lane.axis === "y" ? 100 : lane.fixed),
      true,
    );
});
